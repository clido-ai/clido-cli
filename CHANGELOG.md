...
- **Workflow loop configuration**: Steps can now declare `loop:` configuration with `while`, `until`, and `max_iterations` fields for conditional step repetition. Supports template expressions for dynamic loop conditions.

## [1.0.13] - 2025-01-22

### Fixed

- **Word-aware line wrapping**: Complete rewrite of `wrap_content_lines` to wrap at word boundaries instead of mid-word. Wrapped lines now preserve indentation from the original line, creating properly formatted paragraphs.
