...
pub async fn run_export(session_id: Option<&str>, output: Option<&std::path::Path>) -> anyhow::Result<()> {
    let cwd = env::current_dir().unwrap_or_else(|_| std::path::PathBuf::from("."));
    let id = match session_id {
        Some(s) => s.to_string(),
        None => {
    ...
    for line in &lines {
        match line {
            clido_storage::SessionLine::UserMessage { content, .. } => {
                let text = content.iter().map(|v| v.to_string()).collect::<Vec<_>>().join(" ");
...
pub async fn run_compact(session_id: Option<&str>) -> anyhow::Result<()> {
    ...
    for line in &lines {
        match line {
            clido_storage::SessionLine::UserMessage { content, .. } if all => {
                let t = content.iter().map(|v| v.to_string()).collect::<Vec<_>>().join(" ");
...
pub async fn run_todo(session_id: Option<&str>) -> anyhow::Result<()> {
