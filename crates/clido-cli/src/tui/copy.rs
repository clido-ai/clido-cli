...
pub mod info {
    pub const BARE_SLASH: &str =
        "  Type a message or command — bare '/' alone is not sent";
    pub const EMPTY_HINT: &str =
        "  Type a message to start, or /help for available commands";
    pub const AGENT_NOT_RUNNING: &str =
        "  ✗ Agent is not running — try restarting clido.";
...
pub mod diff {
    pub fn truncated_banner(total_lines: usize, shown: usize) -> String {
        format!(
            "  … Diff truncated: {total_lines} lines total, showing first {shown}."
        )
    }

    pub const TRUNCATE_HINT: &str =
        "  Use git diff or your editor for the full patch.";
}
