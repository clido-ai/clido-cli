        ...
        match action {
            ...
            AgentAction::SwitchProfile(profile_name) => {
                ...
                match AgentSetup::build_with_preloaded_and_store(
                    ...
                    Err(e) => {
                        let _ = event_tx
                            .send(AgentEvent::Err(format!("profile switch: {e}")))
                            .await;
                    }
                }
            }
            AgentAction::Run(prompt) => {
                ...
                let _ = event_tx
                    .send(AgentEvent::RunState(AppRunState::Generating))
                    .await;
                ...
                match result {
                    Ok(text) => {
                        ...
                        if event_tx
                            .send(AgentEvent::Response(text.clone()))
                            .await
                            .is_err()
                        {
                            return;
                ...
                match maybe {
                    ...
                    Some(AgentEvent::Response(text)) => {
                        ...
                        if let Some(rules_path) = app.pending_rules_edit.take() {
                            ...
                            match std::fs::write::<&std::path::Path, &[u8]>(&rules_path, text.as_bytes()) {
                                Ok(_) => {
                                    app.push(ChatLine::Info(format!(
                                        "  ✓ Rules saved to {}",
                                        rules_path.display()
                                ...
                                Err(e) => {
                                    ...
                                    // Restore pending_rules_edit so user can retry
                                    app.pending_rules_edit = Some(rules_path);
                                }
                            }
