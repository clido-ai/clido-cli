...
pub struct ProviderDef {
    ...
    /// Required User-Agent string for providers that gate access by client identity
    /// (e.g. Kimi Code requires `"RooCode/3.0.0"`). Used when no `user_agent` is
    /// explicitly set in the profile — the user can still override it via config.
    pub required_user_agent: Option<&'static str>,
...
pub static PROVIDER_REGISTRY: &[ProviderDef] = &[
    ...
    ProviderDef {
        ...
        extra_headers: &[],
        required_user_agent: Some("RooCode/3.0.0"),
        is_local: false,
        is_anthropic: false,
    },
    ProviderDef {
