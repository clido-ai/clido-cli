...
With harness enabled, the plan strip defaults to **on** and shows task ids, descriptions, and focus (`›` on the focused failing task). Toggle visibility with `/plan` as usual.
